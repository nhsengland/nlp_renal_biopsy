...
class DisagreementAnnotator:
    ...
    def __init__(
        ...
        model_class = RenalBiopsyOllamaQA if backend == 'ollama' else RenalBiopsyLlamaCppQA
    ...
    def analyse_disagreements(
        ...
        for i, (pred1, pred2) in enumerate(
            tqdm(zip(predictions_1, predictions_2),
                 total=n_prototype,
    ...
    def _compare_predictions(
        ...
        for entity, (_, entity_type, _) in self.guidelines.entity_to_info_map.items():
            ...
            if entity_type in ['boolean', 'categorical', 'numerical']:
                ...
                use_llamacpp = (self.backend == 'llamacpp')
                entity_matches[entity] = use_llm_to_compare(value1, value2, llama_cpp=use_llamacpp)
