...
def convert_to_strings(all_jsons):
    final_json = []
    for annotated_json in all_jsons:
        new_json = {
            k: str(v) if v is not None else "null" for k, v in annotated_json.items()
        }
