...
def calculate_entity_accuracy(ground_truth: List[Dict], predictions: List[Dict], entity_to_info_map: Dict, n_prototype: int):
    """
    ...
    for i, (gt_report, pred_report) in enumerate(
        tqdm(zip(ground_truth, predictions), total=n_prototype, desc="Evaluating reports", ncols=80)
    ):
        if i == n_prototype:
            break
        ...
        for entity in report_scores:
            ...
            if report_scores[entity] == 0 and len(results[entity]['incorrect_examples']) < 3:
    ...
    for entity, scores in results.items():
        ...
        if scores['incorrect_examples']:
            ...
            error_examples = f"GT: {example['ground_truth']} → Pred: {example['prediction']}"
